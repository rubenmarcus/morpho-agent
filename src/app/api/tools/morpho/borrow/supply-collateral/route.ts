...
import { encodeMarketParams, generateSupplyCollateralCalldata, generateTransactionPayload } from '../../utils/transaction';
import { ApiResponse, MarketParams, TransactionPayload } from '../../utils/types';

/**
 * API to generate transaction payload for supplying collateral in a Morpho market
 *
...
export async function GET(request: Request) {
  try {
    ...
    if (!morphoAddress || !loanToken || !collateralToken || !oracle || !irm || !lltv || !assets) {
    ...
    // Generate calldata for the supplyCollateral function
    const data = generateSupplyCollateralCalldata(marketParamsEncoded, assets, onBehalf);
    ...
    return NextResponse.json(
      {
        ...
        data: {
          transactionPayload,
          description: `Supplying ${assets} collateral tokens to the ${loanToken}/${collateralToken} market`
        }
      } as ApiResponse<{
    ...
    console.error('Error generating transaction payload for collateral supply:', error);
    return NextResponse.json(
      {
        success: false,
        error: {
          code: 'INTERNAL_ERROR',
          message: 'Failed to generate transaction payload for collateral supply'
        }
      } as ApiResponse<null>,
      { status: 500 }
    );
