...
import { encodeMarketParams, generateRepayCalldata, generateTransactionPayload } from '../../utils/transaction';
import { ApiResponse, MarketParams, TransactionPayload } from '../../utils/types';

/**
 * API to generate transaction payload for repaying a loan in a Morpho market
 *
...
export async function GET(request: Request) {
  try {
    ...
    // Generate calldata for the repay function
    const data = generateRepayCalldata(marketParamsEncoded, assets, onBehalf);
    ...
    return NextResponse.json(
      {
        ...
        data: {
          transactionPayload,
          description: `Repayment of ${assets} tokens in the ${loanToken}/${collateralToken} market`
        }
      } as ApiResponse<{
    ...
    console.error('Error generating transaction payload for repayment:', error);
    return NextResponse.json(
      {
        success: false,
        error: {
